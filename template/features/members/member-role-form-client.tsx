...
const initialState: ActionResult<{ role: string }> = { ok: true, data: { role: "" } }
...
export function MemberRoleFormClient({
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-center gap-2">
      <select name="role" className="h-9 border bg-background px-3 text-sm" defaultValue={role}>
      ...
      {!state.ok ? <span className="text-sm text-destructive">{state.formError}</span> : null}
