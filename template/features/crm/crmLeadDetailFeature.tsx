...
import { CrmLeadDetailFeatureClient } from "@/features/crm/crmLeadDetailFeature.client";
...
export async function CrmLeadDetailFeature({ leadId }: { leadId: string }) {
  const record = await getCrmRecordWorkflow("contact", leadId);
  ...
  return (
    <CrmLeadDetailTemplate
      ...
      toolbar={<CrmLeadDetailFeatureClient />}
    />
  );
}
