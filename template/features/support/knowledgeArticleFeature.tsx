...
import { KnowledgeArticleFeatureClient } from "@/features/support/knowledgeArticleFeature.client";
...
export async function KnowledgeArticleFeature({
  ...
  const record = await getKnowledgeArticleWorkflow(articleId);
  ...
  return (
    <SupportKnowledgeArticleTemplate
      rows={rows}
      stats={[
        {
          label: "Records",
      ...
      toolbar={<KnowledgeArticleFeatureClient />}
    />
  );
}
