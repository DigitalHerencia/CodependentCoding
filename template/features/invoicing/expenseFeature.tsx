...
import { ExpenseFeatureClient } from "@/features/invoicing/expenseFeature.client";
import { getExpenseWorkflow } from "@/lib/workflows/invoicingWorkflows";
...
export async function ExpenseFeature({ expenseId }: { expenseId: string }) {
  const record = await getExpenseWorkflow(expenseId);
  ...
  return (
    <InvoicingExpenseDetailTemplate
      rows={rows}
      stats={[
        {
          label: "Records",
