...
import { AudienceFeatureClient } from "@/features/marketing/audienceFeature.client";
import { getMarketingRecordWorkflow } from "@/lib/workflows/marketingWorkflows";
...
export async function AudienceFeature({ audienceId }: { audienceId: string }) {
  const record = await getMarketingRecordWorkflow("audience", audienceId);
  ...
  return (
    <MarketingAudienceDetailTemplate
      rows={rows}
      stats={[
        {
          label: "Records",
