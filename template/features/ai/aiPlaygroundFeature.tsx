...
import { AiPlaygroundFeatureClient } from "@/features/ai/aiPlaygroundFeature.client";
import { getAiWorkspaceWorkflow } from "@/lib/workflows/aiWorkflows";
...
export async function AiPlaygroundFeature() {
  const workspace = await getAiWorkspaceWorkflow();
  const records = [workspace.configuration, ...workspace.generations];
  ...
  return (
    <AiPlaygroundTemplate
      ...
      toolbar={<AiPlaygroundFeatureClient />}
    />
  );
}
