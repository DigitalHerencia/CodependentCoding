...
import { AdminUserDetailFeatureClient } from "@/features/admin/adminUserDetailFeature.client";
...
export async function AdminUserDetailFeature({ userId }: { userId: string }) {
  ...
  const records = workspace.memberships.filter(
  ...
  return (
    <AdminUserDetailTemplate
      ...
      toolbar={<AdminUserDetailFeatureClient />}
    />
  );
}
