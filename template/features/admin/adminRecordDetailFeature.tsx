...
import { AdminRecordDetailFeatureClient } from "@/features/admin/adminRecordDetailFeature.client";
...
export async function AdminRecordDetailFeature({
  ...
  const records = workspace.records.filter(
    (record) => record.resource === recordId,
  );
  ...
  return (
    <AdminRecordDetailTemplate
      ...
      toolbar={<AdminRecordDetailFeatureClient />}
    />
  );
}
