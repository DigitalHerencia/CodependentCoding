...
export async function approveSocialPostTx(
  ...
  if (
    ...
    throw new Error("A social post requires active, validated variants before approval.");
  }
...
export async function reconcileSocialPublicationTx(
  ...
  for (const result of input.results) {
    await tx.socialVariant.updateMany({
      ...
      data: result.providerPostId
        ...
        : {
            status: "FAILED",
            errorCode: (result.errorCode ?? "Publication failed.").slice(0, 255),
          },
    });
  }
  ...
  const published = variants.filter((variant) => variant.status === "PUBLISHED").length;
