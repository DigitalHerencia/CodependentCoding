...
export async function assignTaskTx(
  ...
  if (result.count !== 1) throw new InvariantViolationError("Task assignment conflicted.");
...
export async function completeMilestoneTx(
  ...
  if (milestone.tasks.some((task) => !["DONE", "CANCELED"].includes(task.status))) {
    throw new InvariantViolationError("A milestone cannot complete with open tasks.");
  }
...
export async function completeProjectTx(
  ...
  if (result.count !== 1) throw new InvariantViolationError("Project completion conflicted.");
