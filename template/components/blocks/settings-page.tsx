...
function readAppearance(): { theme: AppearanceTheme; accentColor: string } {
  const fallback = {
    theme: "system" as AppearanceTheme,
    accentColor: "#3b82f6",
  };
  try {
    ...
    return {
      theme: (["light", "dark", "system"] as const).includes(
...
export function SettingsSurface({
  ...
  return (
    <section className="space-y-4 border border-border bg-[#0b0f12] p-4">
      <div className="border-b border-border pb-3">
        <h2 className="font-mono text-lg font-black text-foreground">
          {title}
        </h2>
        <p className="mt-1 font-mono text-xs text-[#9dc0c8]">{description}</p>
      </div>
      {children ?? (
        <p className="font-mono text-xs text-[#9dc0c8]">
          Settings are loaded from the authoritative server workflow.
        </p>
      )}
    </section>
  );
