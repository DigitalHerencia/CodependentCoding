...
function calculatePosition(
  ...
  left = Math.max(
    padding,
    Math.min(left, viewport.width - popoverRect.width - padding),
  );
  top = Math.max(
    padding,
    Math.min(top, viewport.height - popoverRect.height - padding),
  );
...
function TourOverlay({ targetRect, spotlightPadding }: TourOverlayProps) {
  if (!targetRect) {
    ...
    return <div className="fixed inset-0 z-[9998] bg-black/70" />;
  }
...
function TourPopover({
  ...
  const { currentStep, totalSteps, nextStep, prevStep, close, skip } =
    useTour();
      ...
      const updateRect = () => {
        ...
        if (element) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.warn(
              `[Tour] Step target "${currentStepData.target}" not found in DOM`,
            );
          }
          setTargetRect(null);
        }
