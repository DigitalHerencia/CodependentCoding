...
const Slider = React.forwardRef<HTMLDivElement, SliderProps>(
  (
    ...
    const keyboardDragTimeoutRef = React.useRef<ReturnType<
      typeof setTimeout
    > | null>(null);
    ...
    React.useEffect(() => {
      if (wasControlled.current !== isControlled) {
        if (process.env.NODE_ENV === "development") {
          console.warn(
            "[Slider] Component is changing from " +
    ...
    const [squishes, setSquishes] = React.useState<
      { scaleX: number; scaleY: number }[]
    >(() => actualValue.map(() => ({ scaleX: 1, scaleY: 1 })));
    const [hoveringThumb, setHoveringThumb] = React.useState<number | null>(
      null,
    );
    ...
    React.useEffect(() => {
      const newTargets = actualValue.map(
        (v) => ((v - min) / (max - min)) * 100,
      );
    ...
    const getRangeFillStyle = () => {
      if (isRange) {
        const left = Math.min(
          springs[0]?.position ?? 0,
          springs[1]?.position ?? 0,
        );
