...
function computeLayout(
  ...
  const colWidth =
    maxDepth === 0
      ? 0
      : (width - padding.left - padding.right - nodeWidth) / maxDepth;
  ...
  cols.forEach((ids, col) => {
    ...
    const effectivePadding =
      gaps > 0 ? Math.min(nodePadding, (innerHeight * 0.5) / gaps) : 0;
  ...
  const computedLinks: ComputedLink[] = links
    .map((link) => {
      const src = computedNodes.get(link.source);
      const tgt = computedNodes.get(link.target);
      if (!src || !tgt) return null;

      const srcTotal = (outLinks.get(link.source) || []).reduce(
        (s, l) => s + l.value,
        0,
      );
      const tgtTotal = (inLinks.get(link.target) || []).reduce(
        (s, l) => s + l.value,
        0,
      );
      ...
      const thickness = Math.max(
        2,
        (link.value / Math.max(srcTotal, tgtTotal, 1)) * src.height,
      );
            ...
            {computedNodes.map((node) => (
              <g key={node.id}>
                ...
                {showLabels && (
                  <text
                    x={
                      node.x > width / 2 ? node.x - 6 : node.x + node.width + 6
                    }
