...
function drawSpiral(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const angle =
        (((Math.atan2(dy, dx) + t * 0.002) % (2 * Math.PI)) + 2 * Math.PI) %
        (2 * Math.PI);
...
function drawPulse(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const dist = Math.sqrt(dx * dx + dy * dy);
      const phase =
        (((dist - speed) % ringSpacing) + ringSpacing) % ringSpacing;
      const ringIntensity = Math.pow(
        Math.sin((Math.PI * phase) / ringSpacing),
        2,
      );
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (isH && isV) {
        const wave =
          (Math.sin(
            (c / cols) * Math.PI * 4 + (r / rows) * Math.PI * 2 - t * 0.002,
          ) +
            1) /
          2;
        ...
        const wave =
          Math.sin(
            (c / cols) * Math.PI * 8 + (r / rows) * Math.PI * 4 - t * 0.001,
          ) *
            0.3 +
          0.3;
        grid[r][c] = chars[Math.floor(wave * (chars.length - 1))];
...
function drawSphere(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const onGrid =
        Math.abs(Math.cos(lat * 5)) > 0.93 ||
        Math.abs(Math.cos(lon * 8)) > 0.91;
...
function drawDonut(
  ...
  for (let theta = 0; theta < 2 * Math.PI; theta += 0.07) {
    ...
    for (let phi = 0; phi < 2 * Math.PI; phi += 0.02) {
      ...
      const yp = Math.round(
        cy - K1 * ooz * (cosP * h * sinB + ycomp * cosB) * aspect,
      );
      if (xp < 0 || xp >= cols || yp < 0 || yp >= rows) continue;
...
function drawHelix(
  ...
  for (let phi = 0; phi < turns * 2 * Math.PI; phi += pStep) {
    ...
    // Strand 1
    const x1 = helixR * Math.cos(phi),
      z1 = helixR * Math.sin(phi);
    plot(
      x1,
      y,
      z1,
      Math.max(0.15, Math.cos(phi) * lx + Math.sin(phi) * lz * 0.5 + 0.55),
    );
    // Strand 2 (π offset)
    const x2 = helixR * Math.cos(phi + Math.PI),
      z2 = helixR * Math.sin(phi + Math.PI);
    plot(
      x2,
      y,
      z2,
      Math.max(
        0.15,
        Math.cos(phi + Math.PI) * lx +
          Math.sin(phi + Math.PI) * lz * 0.5 +
          0.55,
      ),
    );
...
function drawDNA(
  ...
  for (let i = 0; i < totalSteps; i++) {
    ...
    const x2 = helixR * Math.cos(angle + strandOffset),
      z2 = helixR * Math.sin(angle + strandOffset);
...
function makeAsciiComponent(
  drawFn: DrawFn,
  defaultCharset: AsciiCharset = "classic",
) {
  // Each exported factory result receives a concrete displayName below.
    ...
    (
      ...
      function combineRef(el: HTMLPreElement | null) {
        ...
        if (typeof ref === "function") ref(el);
          ...
          (ref as React.MutableRefObject<HTMLPreElement | null>).current = el;
      }
            ...
            ? initialLines.map((line, i) => (
                <React.Fragment key={i}>
                  <span
                    ...
                    style={{
                      color: MULTICOLOR_PALETTE[i % MULTICOLOR_PALETTE.length],
                    }}
                  >
                    {line}
                  </span>
...
export const AsciiMatrix = makeAsciiComponent(
  (g, cols, rows, t, chars, state) =>
    drawMatrix(g, cols, rows, t, chars, state!),
  "classic",
);
...
export const AsciiGeodesicDome = makeAsciiComponent(
  drawGeodesicDome,
  "classic",
);
