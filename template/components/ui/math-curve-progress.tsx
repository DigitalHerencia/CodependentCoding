...
const MathCurveProgress = React.forwardRef<
  SVGSVGElement,
  MathCurveProgressProps
>(
  (
    {
      className,
    ...
    const headPoint = React.useMemo(
      () => getPoint(curve, progress),
      [curve, progress],
    );
    const headAngle = React.useMemo(
      () => getAngle(curve, progress),
      [curve, progress],
    );
    const HEAD_SIZE = 8;
