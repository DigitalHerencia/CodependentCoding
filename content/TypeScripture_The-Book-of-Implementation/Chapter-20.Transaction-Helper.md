...
DTO mappers do not call locale-sensitive formatters or invent view labels, badge classifications, or display strings. Features/presentation own those decisions. When an established DTO mapper exists for a selected persistence shape, Actions and Fetchers use it rather than manually serializing the same record again.
...
return tx.invoice.create({
  data: { organizationId, number: (latest._max.number ?? 0) + 1, ...input },
});
```

The exact allocator may differ, but the deciding read and admitted write must not interleave with another allocation for the same scope. A uniqueness constraint remains required for integrity; catching random uniqueness failures is not the intended allocation contract.
