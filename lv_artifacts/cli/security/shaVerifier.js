 ...
 */
export function buildRemediationGuidance(filePath, computedHash, expectedHash) {
  const lines = [
    '',
    '============================================================',
 ...
 * @param {boolean} [options.deleteOnMismatch=true] - Delete the file if hash doesn't match
 * @param {Object} [options.logger] - Optional NDJSON logger instance
 * @returns {Promise<Object>} Verification result with valid, computedHash, expectedHash, error
 */
export async function verifySHA256(filePath, expectedHash, options = {}) {
 ...
 */
export async function cleanupTempFiles(paths, logger) {
 ...
 * @param {Object} [options] - Verification options
 * @returns {Promise<Map<string, Object>>} Map of filename to verification result
 */
export async function verifyMultipleFiles(basePath, checksums, options = {}) {
